...
<link rel="stylesheet" href="/WebContent/bootstrap.css">
<title>Liste de vos notes</title>
</head>
<body>
	<c:choose>
		<c:when test="${ !empty verifStagiaire }">
			<h1>Vous n'etes pas stagiaire, vous n'avez donc pas d'évaluation
			</h1>
		</c:when>
		...
		<c:when test="${ empty verifStagiaire }">
			...
			<table>
				...
				<tbody>
					<c:forEach items="${listeDeModule}" var="module" varStatus="status">
