...
public class Inscrire extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request,
    ...
    request.getRequestDispatcher("/WEB-INF/FormulaireInscription.jsp").forward(request, response);
  }

  /**
   * @see Méthode doPost qui récupère les champs du formulaire et qui
   * insère la personne dans la base de données.
   */
  ...
  /** Verifie que le formulaire est valide : champs requis renseignés,
   * plages de valeur respectées. Positionne isFormValid. Si valide,
   * crée le bean personne reflet de la saisie. Sinon, positionne les
   * messages d'erreur de la forme msgNomDuChamp (ex : msgEmail)
   *
   * @param request
   */
  private void verifierFormulaire(HttpServletRequest request) {
    ...
    if (formIsValid) {
      personne = new Personne(0, civilite, prenom, nom,
          adresse, codePostal, ville, telephone, telephone2,
          email, motPasse, time, estInscrite);
