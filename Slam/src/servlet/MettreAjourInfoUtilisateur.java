...
public class MettreAjourInfoUtilisateur extends HttpServlet {
	...
	private void traitement(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(hasToChangePwd != null && hasToChangePwd.equals("Modifier le mot de passe")) {
...
//		String prenom = request.getParameter("prenom");
		...
		try {
			Personne personneDejaExistante = pDAO.findById(connectedUser.getIdPersonne());
			
			// traiter le changement de mot de passe uniquement si changement activé
			String mdp = "";
			if(request.getParameter("anc_motDePasse") != null) {
				...
				if(!personneDejaExistante.getMotPasse().equals(ancMotPasse) || !nMdp.equals(confMdp)) {
			...
			if(mdp.isEmpty()) {
				pDAO.updatePersonneInfo(personneDejaExistante.getIdPersonne(), adresse, codePostal, ville, telephone, telephone2, mailActuel);
			} else {
				pDAO.updatePersonneInfo(personneDejaExistante.getIdPersonne(), adresse, codePostal, ville, telephone, telephone2, mailActuel, mdp);
			}
			
			personneDejaExistante = pDAO.findById(connectedUser.getIdPersonne());
			
			request.getSession().setAttribute("utilisateur", personneDejaExistante);
